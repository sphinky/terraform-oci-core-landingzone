...
module "lz_groups" {
  ...
  groups_configuration = var.extend_landing_zone_to_new_region == false && var.identity_domain_option != "Use Custom Identity Domain" ? local.groups_configuration : local.empty_groups_configuration
}
...
locals {
  ...
  #-- IAM Identity Domain
  #--------------------------------------------------------------------
  custom_id_domain_name = var.identity_domain_option == "Use Custom Identity Domain" ? data.oci_identity_domain.existing_identity_domain[0].display_name : null
  ...
  groups_configuration = {
    groups : merge(local.iam_admin_group, local.cred_admin_group, local.cost_admin_group,
      ...
      local.database_admin_group, local.exainfra_admin_group,
  ...
  exainfra_admin_group_name      = var.identity_domain_option != "New Identity Domain" ? (!var.extend_landing_zone_to_new_region ? (var.identity_domain_option != "Use Custom Identity Domain" ? ((var.deploy_exainfra_cmp ? (length(var.existing_exainfra_admin_group_name) == 0 && length(trimspace(var.rm_existing_exainfra_admin_group_name)) == 0 ? [module.lz_groups[0].groups[local.exainfra_admin_group_key].name] : (length(regexall("^ocid1.group.oc.*$", var.rm_existing_exainfra_admin_group_name)) > 0 ? ["'${data.oci_identity_group.existing_exainfra_admin_group[var.rm_existing_exainfra_admin_group_name].name}'"] : [for i, v in var.existing_exainfra_admin_group_name : (length(regexall("^ocid1.group.oc.*$", var.existing_exainfra_admin_group_name[i])) > 0 ? "'${data.oci_identity_group.existing_exainfra_admin_group[v].name}'" : "'${v}'")])) : [for grp in var.existing_exainfra_admin_group_name : "'${grp}'"])) : var.deploy_custom_domain_groups == true ? ["'${local.custom_id_domain_name}'/'${local.provided_exainfra_admin_group_name}'"] : [for v in var.rm_existing_id_domain_exainfra_admin_group_name : "'${local.custom_id_domain_name}'/'${trimspace(v)}'"]) : []) : ["'${var.new_identity_domain_name}'/'${local.provided_ag_admin_group_name}'"]
  ag_admin_group_name            = var.identity_domain_option != "New Identity Domain" ? (!var.extend_landing_zone_to_new_region ? (var.identity_domain_option != "Use Custom Identity Domain" ? ((length(var.existing_ag_admin_group_name) == 0 && length(trimspace(var.rm_existing_ag_admin_group_name)) == 0 ? [module.lz_groups[0].groups[local.ag_admin_group_key].name] : (length(regexall("^ocid1.group.oc.*$", var.rm_existing_ag_admin_group_name)) > 0 ? ["'${data.oci_identity_group.existing_ag_admin_group[var.rm_existing_ag_admin_group_name].name}'"] : [for i, v in var.existing_ag_admin_group_name : (length(regexall("^ocid1.group.oc.*$", var.existing_ag_admin_group_name[i])) > 0 ? "'${data.oci_identity_group.existing_ag_admin_group[v].name}'" : "'${v}'")]))) : var.deploy_custom_domain_groups == true ? ["'${local.custom_id_domain_name}'/'${local.provided_ag_admin_group_name}'"] : [for v in var.rm_existing_id_domain_ag_admin_group_name : "'${local.custom_id_domain_name}'/'${trimspace(v)}'"]) : []) : ["'${var.new_identity_domain_name}'/'${local.provided_cred_admin_group_name}'"]
}
